/**
 * OpenAL32.dll — proxy для Lineage 2 Kamael (UE2.5)  v10.5-SA-OCCL (LA 2.6)
 * База: v10.5-SA-OCCL (LA 2.5)
 *
 * LA 2.6:
 *   - OBJ перегенерирован из GeoData с правильными SA координатами
 *     (было: game_x, -game_y, game_z → стало: game_x, game_z, -game_y)
 *   - pos_sim лог исправлен: (x, z, -y)
 *
 * LA 2.5:
 *   - КРИТИЧЕСКИЙ ФИКС: directFlags = 1<<3 (APPLYOCCLUSION), было 1<<2 (APPLYDIRECTIVITY)
 *     Это была главная причина occlusion=1.000 — occlusion вообще не считалась!
 *
 * LA 2.4:
 *   - occlusionType=1, numOcclusionSamples=8, occlusionRadius=2.0
 *   - numOcclusionSamples: 1 → 8 (больше лучей для точности)
 *   - occlusionRadius: 0 → 2.0f (радиус проверки вокруг источника)
 *
 * LA 1.2:
 *   - Убрана инверсия Y координат: OBJ, listener, source — одна система (UE2 as-is)
 *
 * LA 1.1:
 *   - IPLSimulationInputs_t: исправлен layout (204→184 байта)
 */
...
struct SrcState { unsigned int id; float x,y,z; int active; int relative; int diag_log_count; };
...
// LA 2.4: occlusionType=1 (RAYCAST), numOcclusionSamples=8, occlusionRadius=2.0
        ...
        if (g_log && (++diag_tick % 20) == 0) {
            ...
            for (int i = 0; i < SA_MAX_SRC; i++) {
                ...
                fprintf(g_log,
                    ...
                    (double)g_src[i].x, (double)g_src[i].z, (double)(-g_src[i].y),
...
static void sa_scene_init(void)
{
    ...
    const char* obj_paths[] = {
        "oren_scene_dev.obj",
        "steam_audio/oren_scene_dev.obj",
        "../oren_scene_dev.obj",
        "..\\oren_scene_dev.obj",
        NULL
    };
...
// alSource3i — ЛОГИРУЕМ AL_POSITION с дедупликацией + обновляем симулятор
...
extern "C" __declspec(dllexport) void __cdecl alSourcePlay(unsigned int s)
{
    ...
    if (g_scene_ok && g_ipl_src[slot]) {
        ...
        // Применяем occlusion как множитель к gain пользователя
        float occ        = g_occlusion[slot];
        float user_gain  = g_user_gain[slot];
        float final_gain = user_gain * occ;
        typedef void (__cdecl *pfn_sf)(unsigned int, int, float);
        ...
        if (fn) {
            g_internal_gain_set = 1;
            fn(s, 0x100A /*AL_GAIN*/, final_gain);
            g_internal_gain_set = 0;
        }
        if (g_log && occ < 0.95f) {
            log_write("OCCL src=%u slot=%d occ=%.3f gain %.2f->%.2f",
                      s, slot, occ, user_gain, final_gain);
        }
    }
...
extern "C" __declspec(dllexport) void __cdecl alListenerfv(int p, const float* v)
{
    ...
    if (pfn) pfn(p, v);
    if (p == AL_POSITION) {
        // LA 2.7: ЛОГИРУЕМ ВСЕ позиции listener без дедупликации
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID)
{
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        if (g_log) {
            SYSTEMTIME st; GetLocalTime(&st);
            fprintf(g_log,
                "=== OpenAL32 Proxy v10.5-SA-OCCL (LA 2.4 Freya) ===\n"
